...
def get_status_from_config(product_string, da_mapping, linux_mapping, status_mapping):
    ...
    vuln_vendor_lower = vendor.lower()
    vuln_product_lower = product.lower()
    ...
    def check_config_mapping(mapping):
        for key_phrase, value in mapping.items():
            if key_phrase:
                if any(key_phrase in part for part in search_parts if part):
    ...
    if key_s:
        ...
        return status, product_id, "KnownSTATUS Config", key_s

    key_d, product_id = check_config_mapping(da_mapping)
    if key_d:
        return "ДА", product_id, "KnownDA Config", key_d

    key_l, product_id = check_config_mapping(linux_mapping)
    if key_l:
        return "ЛИНУКС", product_id, "KnownLINUX Config", key_l
...
def find_new_strict_matches(vuln_product_str, ppts_df):
    ...
    if ',' in vuln_product_str:
        parts = vuln_product_str.split(',', 1)
        src_vendor, src_product = parts[0].strip(), parts[1].strip()
    ...
    for index, row in ppts_df.iterrows():
        ...
        if match_index >= MIN_OUTPUT_INDEX:
            ...
            matches.append({
                ...
                'matched_word_count': total_word_count
            })
...
def analyze_data(app_instance, config_data):
    global MIN_WORD_LENGTH, MIN_RATIO_SCORE, RATIO_THRESHOLD_2, MIN_OUTPUT_INDEX, WORD_MATCH_COUNT_THRESHOLD
    ...
    try:
        ...
        WORD_MATCH_COUNT_THRESHOLD = int(config_data['word_match_count_threshold'])
        ...
        known_da_mapping = {k.lower().strip(): v.strip() for k, v in known_da_mapping_raw.items()}
        ...
        known_linux_mapping_raw = dict(re.findall(r"(.+?)\s*=\s*([^\n]+)", config_data['known_linux'], re.IGNORECASE))
        known_linux_mapping = {k.lower().strip(): v.strip() for k, v in known_linux_mapping_raw.items()}
        ...
        for index, row in df_vuln.iterrows():
            ...
            config_result = get_status_from_config(product_to_check, known_da_mapping, known_linux_mapping,
                                                   known_status_mapping)
            ...
            main_row = {'№': row.get('№', ''), 'Дата обработки': today_date, 'Ответственный': '', 'Публикация': '',
                        'Статус': status, 'ID ППТС': id_ppts, 'CVE': row.get('CVE', ''), 'CVSS': row.get('CVSS', ''),
            ...
            if config_status:
                ...
                status_for_detailed, id_for_detailed = '', ''
            vuln_info_row = {'№': vuln_counter, 'CVE': row.get('CVE', ''), 'CVSS': row.get('CVSS', ''),
                             'Продукт': product_to_check, 'Источник': row.get('Источник', ''),
                             'Статус из конфига': status_for_detailed, 'ID ППТС из конфига': id_for_detailed,
                             'Matches': new_matches_list, '_status_for_formatting': detailed_status}
            detailed_analysis_list.append(vuln_info_row)
            vuln_counter += 1
        print("\nФормирование отчета Excel...")
        df_main = pd.DataFrame(main_table_data)
        index_explanation = {'Индекс': [0, 1, 2, 3, 4, ''], 'Пояснение': ['Нет совпадений (отсечено)',
                                                                          f'Лучшее слово совпало на >= {MIN_RATIO_SCORE}% ТОЛЬКО в Вендоре или ТОЛЬКО в Продукте',
                                                                          f'Лучшее слово совпало на >= {RATIO_THRESHOLD_2}% ТОЛЬКО в Вендоре или ТОЛЬКО в Продукте',
                                                                          f'Лучшие слова совпали на >= {MIN_RATIO_SCORE}% и в Вендоре, и в Продукте',
                                                                          f'Лучшие слова совпали на >= {RATIO_THRESHOLD_2}% и в Вендоре, и в Продукте',
                                                                          f'Примечание (Индекс вывода >= {MIN_OUTPUT_INDEX})'],
                             'Доп. Инфо': [f'Выводятся только совпадения с индексом >= {MIN_OUTPUT_INDEX}',
                                           'Сортировка по кол-ву слов > T%, затем по %.',
                                           'Сортировка по кол-ву слов > T%, затем по %.',
                                           'Сортировка по кол-ву слов > T%, затем по %.',
                                           'Сортировка по кол-ву слов > T%, затем по %.',
                                           'T - порог для подсчета слов (задается в GUI).']}
        ...
        with pd.ExcelWriter(output_file_final, engine='xlsxwriter') as writer:
            ...
            detailed_headers = ['№', 'CVE', 'CVSS', 'Продукт', 'Источник', 'Статус из конфига', 'ID ППТС из конфига',
                                'ID ППТС (найденный)', 'Совпадение (Имя, Индекс)', 'Доп. Инфо ППТС']
            ...
            for vuln_data in detailed_analysis_list:
                ...
                base_data = [vuln_data['№'], vuln_data['CVE'], vuln_data['CVSS'], vuln_data['Продукт'],
                             vuln_data['Источник'], vuln_data['Статус из конфига'], vuln_data['ID ППТС из конфига'], '',
                             '', '']
            ...
            worksheet_main.set_column('A:J', 15)
            ...
            worksheet_index.set_column('A:D', 40)
...
class Application(tk.Tk):
    def __init__(self):
        ...
        self.file_vars = {'file_vulnerabilities': tk.StringVar(value=""), 'file_ppts_local': tk.StringVar(value=""),
                          'file_ppts_general': tk.StringVar(value=""), 'status_config_file': tk.StringVar(value=""),
    ...
    def _bind_text_widgets(self, widget):
        def _copy(event):
            try:
                widget.clipboard_clear(); widget.clipboard_append(widget.selection_get())
            except:
                pass
            return "break"
        ...
        def _select_all(event):
            try:
                widget.tag_add(tk.SEL, "1.0", tk.END); widget.mark_set(tk.INSERT, "1.0")
            except:
                pass
            return "break"
    ...
    def create_widgets(self):
        ...
        tk.Label(settings_frame, text="Порог слов >T% (60%):").grid(row=6, column=0, sticky="w");
        self.word_count_thresh_entry = tk.Entry(settings_frame, width=5);
        self.word_count_thresh_entry.insert(0, "60");
        self.word_count_thresh_entry.grid(row=6, column=1, sticky="w")
        config_data_frame = tk.LabelFrame(config_frame, text="Конфигурационные данные", padx=5, pady=5);
        ...
        tk.Label(config_data_frame, text="[KnownLINUX] (ID):").pack(fill=tk.X);
        self.known_linux_text = scrolledtext.ScrolledText(config_data_frame, height=5);
        self.known_linux_text.pack(fill=tk.X);
        self._bind_text_widgets(self.known_linux_text)
    ...
    def _select_file(self, var_key, is_save, callback=None, filetypes=None):
        ...
        if is_save:
            file_path = filedialog.asksaveasfilename(initialdir=initial_dir, defaultextension=".xlsx",
                                                     filetypes=[("Excel files", "*.xlsx")],
                                                     initialfile=os.path.basename(
                                                         initial_file) if initial_file else "output_report.xlsx")
        else:
            final_filetypes = filetypes if filetypes else [("Excel files", "*.xlsx"), ("All files", "*.*")]
            file_path = filedialog.askopenfilename(initialdir=initial_dir, filetypes=final_filetypes)
        if file_path:
    ...
    def _load_status_config(self, file_path):
        ...
        try:
            ...
            def get_section_text(section_name):
                if section_name.upper() in config_parser:
                    return "\n".join(
                        [f"{k} = {v}" if v is not None else k for k, v in config_parser.items(section_name.upper())])
                return ""
            ...
            linux_text = get_section_text('KnownLINUX');
            self.known_linux_text.delete('1.0', tk.END);
            self.known_linux_text.insert(tk.END, linux_text)
    ...
    def start_analysis_thread(self):
        config_data = {'file_vulnerabilities': self.file_vars['file_vulnerabilities'].get(),
                       ...
                       'ppts_local_columns': self.cols_l_entry.get(), 'ppts_general_columns': self.cols_g_entry.get(),
                       'min_word_length': self.min_len_entry.get(), 'min_ratio_score': self.ratio_1_entry.get(),
                       'ratio_threshold_2': self.ratio_2_entry.get(), 'min_output_index': self.min_idx_entry.get(),
                       'word_match_count_threshold': self.word_count_thresh_entry.get(),
                       'known_status': self.known_status_text.get('1.0', tk.END),
                       'known_da': self.known_da_text.get('1.0', tk.END),
                       'known_linux': self.known_linux_text.get('1.0', tk.END)}
